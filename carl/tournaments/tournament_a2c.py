...
from carl.agents.tensorflow.A2C import A2CAgent
...
filenames = [f for f in os.listdir(os.path.join('models', 'A2C')) if f.endswith('.h5')]
filepaths = [os.path.join('models', 'A2C', filename) for filename in filenames]
...
env = Environment(circuits, n_agents, names=modelnames,
    action_type='continueous', n_sensors=5, fov=np.pi*200/180, car_width=0.1, speed_unit=0.05)

agents = [A2CAgent(env.action_space) for _ in range(n_agents)]
